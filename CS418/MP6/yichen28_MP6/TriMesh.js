...
class TriMesh{   
   ...
   */
    ...
    */
    ...
    {    
        ...
        lines.forEach(line =>{
            ...
            if(values[0] === 'v') {
                this.vBuffer.push(values[1]);
                this.vBuffer.push(values[2]);
                this.vBuffer.push(values[3]);
            }
            else if(values[0] === 'f') {
                // console.log(values)
                this.fBuffer.push(values[1] - 1);
                this.fBuffer.push(values[2] - 1);
                this.fBuffer.push(values[3] - 1);
            }
    ...
    */    
    ...
    canonicalTransform(){
      //Your code here
      var centerX = (this.minXYZ[0] + this.maxXYZ[0]) / 2
      var centerY = (this.minXYZ[1] + this.maxXYZ[1]) / 2
      var centerZ = (this.minXYZ[2] + this.maxXYZ[2]) / 2
      var vecT = glMatrix.vec3.fromValues(-centerX, -centerY, -centerZ)
      var T = glMatrix.mat4.create();
      var S = glMatrix.mat4.create();
      glMatrix.mat4.fromTranslation(T, vecT);
      var L = Math.max(this.maxXYZ[0] - this.minXYZ[0], Math.max(this.maxXYZ[1] - this.minXYZ[1], this.maxXYZ[2] - this.minXYZ[2]))
      var vecS = glMatrix.vec3.fromValues(1/L, 1/L, 1/L)
      glMatrix.mat4.fromScaling(S, vecS)
