...
/** @global min Z value of the terrain */
var minZ = 0;
/** @global max Z value of the terrain */
var maxZ = 0;
...
var camOrientation = glMatrix.quat.fromValues(0.0, 1.0, 0.0, 1.0);    
/** @global The camera's current speed in the forward direction*/
var camSpeed = 0.001;                                                
/** @global The camera's initial view direction */
var camInitialDir = glMatrix.vec3.fromValues(1.0, 2.0, 0.0);           
/** @global The initial up vector*/
var initialUp = glMatrix.vec3.fromValues(0.0, 1.0, 0.0);
/** @global The current up vector*/
var up = glMatrix.vec3.fromValues(0.0, 1.0, 0.0);
/** @global Flag to decide whether to generate fog*/
var fog = 0;
/**
...
function startup() {
  ...
  myTerrain = new Terrain(148, -1, 1, -1, 1);
  this.minZ = myTerrain.getMinElevation();
  this.maxZ = myTerrain.getMaxElevation();
...
function draw() {
  ...
  // const lookAtPt = glMatrix.vec3.fromValues(0.0, 0.5, -1.0);
  // const eyePt = glMatrix.vec3.fromValues(0.0, -1.0, 2.5);
  ...
  glMatrix.vec3.transformQuat(up, initialUp, camOrientation);
  ...
  fog = document.getElementById("fog").checked ? 1 : 0;
  console.log(fog);
 ...
 */
 ...
 if (keys["Escape"]) {
  ...
  camOrientation = glMatrix.quat.fromValues(0.0, 1.0, 0.0, 1.0);
  up = glMatrix.vec3.fromValues(0.0, 1.0, 0.0);
  camSpeed = 0.001; 
}
 var orientationDelta = glMatrix.quat.create(); 
 var forwadDirection = glMatrix.vec3.create();
 ...
 //position
 glMatrix.vec3.transformQuat(forwadDirection,camInitialDir,camOrientation);
 glMatrix.vec3.normalize(forwadDirection, forwadDirection);
 glMatrix.vec3.scale(deltaPosition, forwadDirection, camSpeed);
